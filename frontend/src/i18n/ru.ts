...
const ru: Translations = {
  ...
  step3: {
    ...
    timeout: 'Таймаут (с)',
    placeholderPatterns: 'Паттерны плейсхолдеров',
    placeholderPatternPlaceholder: 'напр. 📷\\s*Фото\\s*\\d+',
    addPattern: 'Добавить',
    placeholderPatternsDesc: 'Регулярные выражения (без учёта регистра) для поиска плейсхолдеров изображений в тексте абзацев. Добавьте паттерны, если документ использует нестандартные метки.',
    hints: {
      ...
      outputFormats: 'Выберите формат итогового файла.',
      ...
      imageSources: 'Провайдер для поиска изображений. Wikimedia бесплатный; для Pexels и Unsplash нужны API-ключи.',
      ...
      placeholderPatterns: 'Дополнительные регулярные выражения (без учёта регистра) для распознавания плейсхолдеров изображений. Полезно для документов с нестандартными метками, например с префиксами-эмодзи.',
    },
  },
